...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $is_popular = isset($_POST['is_popular']) ? 1 : 0; // Checkbox for marking as popular
    $image = $_FILES['image']['name'];
    $target = "../images/" . basename($image);

    if (move_uploaded_file($_FILES['image']['tmp_name'], $target)) {
        $stmt = $conn->prepare("INSERT INTO blog (title, content, image, is_popular) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("sssi", $title, $content, $image, $is_popular);
        $stmt->execute();
        $stmt->close();
        echo '<script>
...
<head>
    ...
    <title>Add Blog - Career Compass</title>
